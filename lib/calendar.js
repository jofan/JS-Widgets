  ...
  constructor: App.widget.Calendar,
	...
	*/
	...
	update: function() {
		...
		var captionStr = '<caption><a href="#" class="prevMonth navLink">&larr;</a>' + this.MONTHS[this.month] + ' ' + this.year + '<a href="#" class="nextMonth navLink">&rarr;</a></caption>';
	...
	*/
	show: function(ctrl) {
	  ...
	  var pos = ctrl.position();
		var date = ctrl.val();
		if(date !== ''){
			// Full version should check for date format
			var arrDate = date.split('.');
			this.setDateInfo(parseInt(arrDate[2]), parseInt((arrDate[1] - 1)), parseInt(arrDate[0]));
			this.update();
		}
